...
pub async fn upload_image(url: &str) -> Result<String, String> {
    ...
    let resp = client
        .post("http://127.0.0.1:2233/image/upload")
    ...
    match resp {
        Ok(resp) => {
            ...
            match json_resp {
                Ok(json_resp) => match serde_json::from_value::<ApiUploadResponse>(json_resp) {
...
pub async fn get_image_base64(image_id: &str) -> Result<String, String> {
    let client = reqwest::Client::new();
    let resp = client
        .get(format!("http://127.0.0.1:2233/image/{}", image_id).as_str())
        .send()
        .await;
    match resp {
        Ok(resp) => {
...
pub async fn memes(
    ...
    meme_options: serde_json::Value,
) -> Result<String, String> {
    let images: Vec<ApiImage> = image_ids
        .into_iter()
        .map(|id| ApiImage {
            name: id.clone(),
            id,
    ...
    let resp = client
        .post(format!("http://127.0.0.1:2233/memes/{}", key).as_str())
        .json(&req_json)
        .send()
        .await;
    match resp {
        Ok(resp) => {
            ...
            let json_resp: Result<serde_json::Value, reqwest::Error> = resp.json().await;
            let json_resp = if let Ok(value) = json_resp {
                value
            } else {
                return Err(format!(
                    "Failed to parse response JSON: {}",
                    json_resp.unwrap_err()
                ));
            };
            match json_resp.get("image_id") {
                Some(image_id_value) => match image_id_value.as_str() {
                    Some(image_id) => return Ok(image_id.to_string()),
                    None => return Err("Invalid image_id format".to_string()),
                },
                None => return Err(format!("image_id not found in response: {}", json_resp)),
            }
        }
        Err(err) => {
            return Err(err.to_string());
